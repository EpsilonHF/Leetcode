...
After adding 1 to A[0], the array is [2,2,3,4], and the sum of even 
values is 2 + 2 + 4 = 8.
After adding -3 to A[1], the array is [2,-1,3,4], and the sum of even 
values is 2 + 4 = 6.
...
class Solution:
    def sumEvenAfterQueries(self, A: List[int], queries: List[List[int]]) -> List[int]:
        ret = []
        num = sum(i for i in A if not i % 2)
        for query in queries:
            val, index = query
            old = A[index]
            A[index] += val
            if old % 2 and not A[index] % 2:
                num += A[index]
            elif not old % 2 and not A[index] % 2:
                num += val
            elif not old % 2 and A[index] % 2:
                num -= old
